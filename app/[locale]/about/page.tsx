...
export default async function AboutPage({
  ...
  return (
    <main className="page-shell space-y-10">
      <SectionHeading
        eyebrow={dictionary.common.club}
        title={dictionary.about.title}
        body={localizeText(locale, data.settings.mission)}
      />
      <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="panel-dark p-8">
          <p className="eyebrow">{dictionary.common.identity}</p>
          <p className="mt-4 text-lg leading-8 text-white/70">{localizeText(locale, data.settings.tagline)}</p>
          <div className="mt-8 grid gap-4 sm:grid-cols-3">
            <StatCard label={dictionary.about.wins} value={`${data.teamStats.wins}`} accent />
            <StatCard label={dictionary.about.losses} value={`${data.teamStats.losses}`} />
            <StatCard label={dictionary.about.runsDiff} value={`${data.teamStats.runsScored - data.teamStats.runsAllowed}`} />
          </div>
        </div>
        <div className="panel p-8">
          <p className="eyebrow">{dictionary.common.visualSystem}</p>
          <div className="mt-5 grid gap-4">
            <div className="rounded-3xl bg-gold p-6 text-ink">
              <p className="text-xs uppercase tracking-[0.3em]">{dictionary.common.primary}</p>
            ...
            <div className="rounded-3xl border border-white/10 bg-black p-6">
              <p className="text-xs uppercase tracking-[0.3em] text-white/45">{dictionary.common.secondary}</p>
